...
class App extends Component {
  ...
  pickRandomWords(myArray) {
    var random;
    for (let i=0; i < 10; i++) {
      random[i] = this.state.myPoemWords[Math.floor(Math.random() * this.state.myPoemWords.length)];
    }
    return random;
  }
  render () {
      ...
      <div className="App"> 
              ...
              <div className="container">
                <div className="row"> 
                  ...
                  {
            // u put "todo.completed" into the strikethrough, which sends the
            // boolean to the function, which then returns the strikethrough css code to update the dom
            this.state.todos.map((todo) => 
              <Todo key={todo.id} id={todo.id} text={todo.text} isComplete={todo.completed} textDecoration={this.strikethrough(todo.completed)} />
            )
          }
                </div>
              </div>
